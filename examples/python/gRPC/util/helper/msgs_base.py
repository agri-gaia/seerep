...
class MsgsBase(ABC, Generic[T]):
    ...
    def _validate_enum(self, enum):
        ...
        if not type(enum) == enum_type:
            raise KeyError(
                f"the used enum of {self.__class__} is of type {enum_type} not of type {type(enum)}"
            )
    ...
    def __init__(self, channel: Channel, enum_types: Set[FrozenEnum] = set()):
        ...
        self._enum_func_mapping: Dict[
            FrozenEnum, MsgsFunctions
        ] = self._set_enum_func_mapping()

        if len(self._enum_func_mapping) < 1:
            raise KeyError(
                f"dict returned by implemented _set_enum_func_mapping() cannot be empty!"
            )
    ...
    def _assemble_components(self):
        ...
        for enum_type in all_enum_types:
            if enum_type in self.active_enums:
                self._set_component(
                    enum_type, self._enum_func_mapping[enum_type].active_function()
                )
            else:
                self._set_component(
                    enum_type, self._enum_func_mapping[enum_type].default_function()
                )
