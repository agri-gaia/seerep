...
{
Hdf5FbPointCloud::Hdf5FbPointCloud(std::shared_ptr<HighFive::File>& file,
                                   std::shared_ptr<std::mutex>& writeMtx)
  : Hdf5CoreGeneral(file, writeMtx), Hdf5FbGeneral(file, writeMtx)
{
}

void Hdf5FbPointCloud::writePointCloud2(const std::string& id,
                                        const seerep::fb::PointCloud2& pcl)
{
  const std::scoped_lock lock(*m_write_mtx);

  const std::string hdf5GroupPath =
      seerep_hdf5_core::Hdf5CorePointCloud::HDF5_GROUP_POINTCLOUD + "/" + id;
  ...
  writeLabelsFb(seerep_hdf5_core::Hdf5CorePointCloud::HDF5_GROUP_POINTCLOUD, id,
                pcl.labels());

  HighFive::DataSpace dataSpace{ pcl.data()->size() };
  std::shared_ptr<HighFive::DataSet> payloadPtr =
      getHdf5DataSet<uint8_t>(hdf5GroupPath + "/points", dataSpace);
  payloadPtr->write(std::move(pcl.data()->data()));
  m_file->flush();
}

void Hdf5FbPointCloud::writeBoundingBox(
    const std::string& uuid, const seerep_core_msgs::Point& min_corner,
    const seerep_core_msgs::Point& max_corner)
{
  std::shared_ptr<HighFive::Group> dataGroupPtr = getHdf5Group(
  ...
  writeAttributeToHdf5<std::vector<float>>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::BOUNDINGBOX, bb);
  m_file->flush();
}
...
Hdf5FbPointCloud::computeBoundingBox(const seerep::fb::PointCloud2& pcl)
{
  ...
  const float* x = reinterpret_cast<const float*>(pcl.data()->data() +
                                                  getChannelOffset(pcl, "x"));
  const float* y = reinterpret_cast<const float*>(pcl.data()->data() +
                                                  getChannelOffset(pcl, "y"));
  const float* z = reinterpret_cast<const float*>(pcl.data()->data() +
                                                  getChannelOffset(pcl, "z"));
...
void Hdf5FbPointCloud::writeGeneralAttributes(
    std::shared_ptr<HighFive::Group>& dataGroupPtr,
    const seerep::fb::PointCloud2& cloud)
{
  writeAttributeToHdf5<uint32_t>(*dataGroupPtr,
                                 seerep_hdf5_core::Hdf5CorePointCloud::HEIGHT,
                                 cloud.height());
  writeAttributeToHdf5<uint32_t>(*dataGroupPtr,
                                 seerep_hdf5_core::Hdf5CorePointCloud::WIDTH,
                                 cloud.width());
  writeAttributeToHdf5<bool>(*dataGroupPtr,
                             seerep_hdf5_core::Hdf5CorePointCloud::IS_BIGENDIAN,
                             cloud.is_bigendian());
  writeAttributeToHdf5<uint32_t>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::POINT_STEP,
      cloud.point_step());
  writeAttributeToHdf5<uint32_t>(*dataGroupPtr,
                                 seerep_hdf5_core::Hdf5CorePointCloud::ROW_STEP,
                                 cloud.row_step());
  writeAttributeToHdf5<bool>(*dataGroupPtr,
                             seerep_hdf5_core::Hdf5CorePointCloud::IS_DENSE,
                             cloud.is_dense());
}
...
Hdf5FbPointCloud::readPointCloud2(const std::string& id, const bool withoutData)
{
  ...
  std::shared_ptr<HighFive::Group> dataGroupPtr =
      getHdf5Group(hdf5GroupPath, false);
  ...
  try
  {
    ...
    readGeneralAttributes(id, dataGroupPtr, height, width, pointStep, rowStep,
                          isBigendian, isDense);
  }
  ...
  auto headerAttributesOffset =
      readHeaderAttributes(builder, *dataGroupPtr, id);
  ...
  auto pointFieldsVectorOffset =
      readPointFieldsOffset(builder, names, offsets, counts, datatypes);

  flatbuffers::uoffset_t dataOffset;
  uint8_t* data;
  if (!withoutData)
  {
    dataOffset = builder.CreateUninitializedVector(height * width * pointStep,
                                                   sizeof(uint8_t), &data);
    HighFive::DataSet payloadDataset =
        m_file->getDataSet(hdf5GroupPath + "/points");
    payloadDataset.read(data);
  }

  auto labelsOffset = readLabels(
      seerep_hdf5_core::Hdf5CorePointCloud::HDF5_GROUP_POINTCLOUD, id, builder);
...
void Hdf5FbPointCloud::readGeneralAttributes(
    const std::string& id, std::shared_ptr<HighFive::Group> dataGroupPtr,
    uint32_t& height, uint32_t& width, uint32_t& pointStep, uint32_t& rowStep,
    bool& isBigendian, bool& isDense)
{
  height = readAttributeFromHdf5<uint32_t>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::HEIGHT, id);
  width = readAttributeFromHdf5<uint32_t>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::WIDTH, id);
  pointStep = readAttributeFromHdf5<uint32_t>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::POINT_STEP, id);
  rowStep = readAttributeFromHdf5<uint32_t>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::ROW_STEP, id);
  isBigendian = readAttributeFromHdf5<bool>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::IS_BIGENDIAN, id);
  isDense = readAttributeFromHdf5<bool>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::IS_DENSE, id);
}

void Hdf5FbPointCloud::readPointFields(
    const std::string& id, std::shared_ptr<HighFive::Group> dataGroupPtr,
    std::vector<std::string>& names, std::vector<uint32_t>& offsets,
    std::vector<uint32_t>& counts, std::vector<uint8_t>& datatypes)
{
  names = readAttributeFromHdf5<std::vector<std::string>>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::FIELD_NAME, id);
  offsets = readAttributeFromHdf5<std::vector<uint32_t>>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::FIELD_OFFSET, id);
  counts = readAttributeFromHdf5<std::vector<uint32_t>>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::FIELD_COUNT, id);
  datatypes = readAttributeFromHdf5<std::vector<uint8_t>>(
      *dataGroupPtr, seerep_hdf5_core::Hdf5CorePointCloud::FIELD_DATATYPE, id);
}

flatbuffers::Offset<
    flatbuffers::Vector<flatbuffers::Offset<seerep::fb::PointField>>>
Hdf5FbPointCloud::readPointFieldsOffset(
    flatbuffers::grpc::MessageBuilder& builder, std::vector<std::string>& names,
    ...
    std::vector<uint8_t>& datatypes)
{
  ...
  for (size_t i = 0; i < names.size(); i++)
  {
    ...
    pointField.add_datatype(
        static_cast<seerep::fb::Point_Field_Datatype>(datatypes.at(i)));
...
uint32_t
Hdf5FbPointCloud::getChannelOffset(const seerep::fb::PointCloud2& pcl,
                                   const std::string& channel_name) const
{
  ...
  throw std::runtime_error("Requested field: " + channel_name +
                           " does not exist!");
}

uint32_t Hdf5FbPointCloud::getRgbaOffset(const std::string& channel_name,
