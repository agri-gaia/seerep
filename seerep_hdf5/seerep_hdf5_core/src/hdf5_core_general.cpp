...
{
Hdf5CoreGeneral::Hdf5CoreGeneral(std::shared_ptr<HighFive::File>& file,
...
std::vector<std::string> Hdf5CoreGeneral::getGroupDatasets(const std::string& id)
{
  ...
  else
  {
    ...
    if (std::find(rootObjects.begin(), rootObjects.end(), id) !=
        rootObjects.end())
    {
...
{
  return readAttributeFromHdf5<std::string>(*m_file, PROJECTNAME,
                                            m_file->getName());
}
...
{
  return readFrameId(*m_file, PROJECTFRAMEID,
...
const std::optional<std::string> Hdf5CoreGeneral::readVersion()
{
  ...
  {
    version =
        readAttributeFromHdf5<std::string>(*m_file, VERSION, m_file->getName());
  }
        ...
        labelsCategoryMap)
{
  ...
  readLabels(datatypeGroup, uuid, labelCategories, labelsPerCategory,
...
void Hdf5CoreGeneral::readLabels(
    const std::string& datatypeGroup, const std::string& uuid,
    ...
    std::vector<std::string>& datumaroJsonPerCategory)
{
  ...
  for (const std::string& category : labelCategories)
  {
    ...
    std::vector<std::string> labels = readDataset<std::vector<std::string>>(
        id + "/" + LABEL + "_" + category);
    std::vector<int> labelsIdDatumaro = readDataset<std::vector<int>>(
        id + "/" + LABEL_ID_DATUMARO + "_" + category);
    std::vector<std::string> instances = readDataset<std::vector<std::string>>(
        id + "/" + LABELINSTANCES + "_" + category);
    std::vector<int> instancesIdDatumaro = readDataset<std::vector<int>>(
        id + "/" + LABELINSTANCES_ID_DATUMARO + "_" + category);
    datumaroJsonPerCategory.push_back(
        readDataset<std::string>(id + "/" + DATUMARO_JSON + "_" + category));

    if (labels.size() != labelsIdDatumaro.size())
    {
      const std::string errorMsg =
          "Unequal size of labels and datumaro label ids in category: " +
          category;
    ...
    {
      if (!hasEqualSize(labels, labelsIdDatumaro, instances,
                        instancesIdDatumaro))
      {
        const std::string errorMsg =
            "unequal size of instances and labels in: " + category;
        BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::error)
            << errorMsg;
        throw std::runtime_error(errorMsg);
      }
    ...
    for (size_t i = 0; i < labels.size(); i++)
    {
      ...
      if (addInstances)
      {
        ...
        catch (std::runtime_error&)
        {
          ...
          BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::warning)
              << "Could not convert instance uuid string to uuid: "
              << instances.at(i);
        }
      }
...
void Hdf5CoreGeneral::writeLabels(
    const std::string& datatypeGroup, const std::string& uuid,
    const std::vector<seerep_core_msgs::LabelCategory>& LabelCategory)
{
  ...
  for (auto labels : LabelCategory)
  {
    HighFive::DataSet datasetLabels = m_file->createDataSet<std::string>(
        id + "/" + seerep_hdf5_core::Hdf5CoreGeneral::LABEL + "_" +
            labels.category,
    ...
    HighFive::DataSet datasetLabelsIdDatumaro = m_file->createDataSet<int>(
        id + "/" + seerep_hdf5_core::Hdf5CoreGeneral::LABEL_ID_DATUMARO + "_" +
            labels.category,
    ...
    HighFive::DataSet datasetInstances = m_file->createDataSet<std::string>(
        id + "/" + seerep_hdf5_core::Hdf5CoreGeneral::LABELINSTANCES + "_" +
            labels.category,
    ...
    HighFive::DataSet datasetInstancesIdDatumaro = m_file->createDataSet<int>(
        id + "/" +
            seerep_hdf5_core::Hdf5CoreGeneral::LABELINSTANCES_ID_DATUMARO +
            "_" + labels.category,
    ...
    HighFive::DataSet datasetDatumaroJson = m_file->createDataSet<std::string>(
        id + "/" + seerep_hdf5_core::Hdf5CoreGeneral::DATUMARO_JSON + "_" +
            labels.category,
...
const std::string
Hdf5CoreGeneral::tf2_frame_id(const std::string& frame_id) const
{
        ...
        float, 3, boost::geometry::cs::cartesian>>& aabb)
{
  ...
  std::vector<float> aabbPoints{
    aabb.min_corner().get<0>(), aabb.min_corner().get<1>(),
    aabb.min_corner().get<2>(), aabb.max_corner().get<0>(),
    aabb.max_corner().get<1>(), aabb.max_corner().get<2>()
  };

  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::trace)
      << "write AABB as attribute";
...
bool Hdf5CoreGeneral::hasAABB(const std::string& datatypeGroup,
...
void Hdf5CoreGeneral::checkExists(const std::string& id)
{
  if (!m_file->exist(id))
  {
    ...
    throw std::invalid_argument("id " + id + " does not exist in file " +
                                m_file->getName());
  }
}
...
bool Hdf5CoreGeneral::exists(const std::string& path) const
{
  if (!m_file->exist(path))
  {
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::warning)
        << "HDF5 path: " << path
        << " does not exist in file: " << m_file->getName();
    return false;
  }
  return true;
}
...
Hdf5CoreGeneral::getHdf5Group(const std::string& group_path, bool create)
{
    ...
    const seerep_core_msgs::GeodeticCoordinates& geo_coordinates)
{
  ...
  writeAttributeToHdf5<double>(*m_file, GEODETICLOCATION_LONGITUDE,
                               geo_coordinates.longitude);
  m_file->flush();
}

std::optional<seerep_core_msgs::GeodeticCoordinates>
Hdf5CoreGeneral::readGeodeticLocation()
{
  ...
  try
  {
    ...
    geocoords.latitude = readAttributeFromHdf5<double>(
        *m_file, GEODETICLOCATION_LATITUDE, m_file->getName());
    geocoords.longitude = readAttributeFromHdf5<double>(
        *m_file, GEODETICLOCATION_LONGITUDE, m_file->getName());
  }
  catch (const std::invalid_argument& e)
  {
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::warning)
        << "Geographic coordinates of the project " << m_file->getName()
        << " could not be read!";
    return std::nullopt;
  }
  return geocoords;
...
Hdf5CoreGeneral::getHdf5DataSet(const std::string& hdf5DataSetPath)
{
  if (exists(hdf5DataSetPath))
  {
    return std::make_shared<HighFive::DataSet>(
        m_file->getDataSet(hdf5DataSetPath));
  }
  return nullptr;
}

void Hdf5CoreGeneral::getLabelCategories(
    std::string id, std::string labelType,
