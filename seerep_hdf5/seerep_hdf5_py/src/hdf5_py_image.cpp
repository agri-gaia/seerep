...
std::vector<std::string> Hdf5PyImage::getImages()
{
  ...
  const HighFive::Group& cloudsGroup =
      m_file->getGroup(seerep_hdf5_core::Hdf5CoreImage::HDF5_GROUP_IMAGE);
  return cloudsGroup.listObjectNames();
}

void Hdf5PyImage::writeImage(
    const std::string& uuid, const std::string& frameId, int64_t seconds,
    int32_t nanos, uint32_t sequence, const py::array& image,
    ...
    const std::vector<CategorizedBoundingBoxLabel<2>>& bbLabels)
{
  ...
  else
  {
    throw std::invalid_argument(
        "image must have either 1 (MONO), 3 (RGB) or 4 (RGBA) channels");
  }

  std::string hdf5GroupPath = getHdf5GroupPath(uuid);
  std::string hdf5DatasetPath = getHdf5DataSetPath(uuid);

  HighFive::DataSpace imageDataSpace(
      { imageBuffInfo.shape[0] * imageBuffInfo.shape[1] * numChannels });
  ...
  if (image.dtype().equal(py::dtype::of<uint8_t>()))
  {
    ...
    auto imageDatasetPtr =
        getHdf5DataSet<uint8_t>(hdf5DatasetPath, imageDataSpace);
    ...
    imageDatasetPtr->write(std::vector<uint8_t>(
        typedImage.data(), typedImage.data() + imageBuffInfo.shape[0] *
                                                   imageBuffInfo.shape[1] *
                                                   numChannels));
  }
  else if (image.dtype().equal(py::dtype::of<uint16_t>()))
  {
    ...
    auto imageDatasetPtr =
        getHdf5DataSet<uint16_t>(hdf5DatasetPath, imageDataSpace);
    encoding = "rgb16";
  ...
  {
    throw std::invalid_argument(
        "image data dtype needs to be either 8 or 16 bit unsigned integer");
  }
  ...
  writeAttributeToHdf5(*dataGroupPtr,
                       ...
                       nanos);
  writeAttributeToHdf5(*dataGroupPtr,
                       seerep_hdf5_core::Hdf5CoreGeneral::HEADER_FRAME_ID,
                       frameId);
  writeAttributeToHdf5(*dataGroupPtr,
                       seerep_hdf5_core::Hdf5CoreGeneral::HEADER_SEQ, sequence);

  seerep_hdf5_core::ImageAttributes image_attributes = {
    static_cast<uint32_t>(imageBuffInfo.shape[0]),
    static_cast<uint32_t>(imageBuffInfo.shape[1]),
    static_cast<uint32_t>(imageBuffInfo.shape[1] * numChannels * channelSize),
    encoding, false
  };
...
Hdf5PyImage::readImage(const std::string& uuid)
{
  ...
  if (imageAttributes.encoding.compare("mono8") == 0 ||
      imageAttributes.encoding.compare("rgb8") == 0 ||
      imageAttributes.encoding.compare("rgba8") == 0)
  {
    image =
        py::array_t<uint8_t>({ imageAttributes.height, imageAttributes.width,
                               imageAttributes.step / imageAttributes.width });
  ...
  else if (imageAttributes.encoding.compare("mono16") == 0 ||
           imageAttributes.encoding.compare("rgb16") == 0 ||
           imageAttributes.encoding.compare("rgba16") == 0)
  {
    image = py::array_t<uint16_t>(
        { imageAttributes.height, imageAttributes.width,
          imageAttributes.step / imageAttributes.width / 2 });
