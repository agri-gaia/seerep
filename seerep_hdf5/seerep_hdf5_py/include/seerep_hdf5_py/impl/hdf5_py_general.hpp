  ...
  for (auto& categoryLabels : bbLabels)
  {
    if (!categoryLabels.labels_.empty())
    {
      ...
      for (auto& instanceLabel : categoryLabels.labels_)
      {
        ...
        boxWithRotation.reserve(2 * NumDimensions +
                                instanceLabel.bbRotation_.size());
      ...
      auto datasetLabels = getHdf5DataSet<std::string>(
          dataGroupId + "/" + seerep_hdf5_core::Hdf5CoreGeneral::LABELBB + "_" +
      ...
      auto datasetLabelConfidences = getHdf5DataSet<float>(
          dataGroupId + "/" +
              seerep_hdf5_core::Hdf5CoreGeneral::LABELBBCONFIDENCES + "_" +
              categoryLabels.category_,
      ...
      auto datasetInstances = getHdf5DataSet<std::string>(
          dataGroupId + "/" +
              seerep_hdf5_core::Hdf5CoreGeneral::LABELBBINSTANCES + "_" +
              categoryLabels.category_,
          instancesDataSpace);
      datasetInstances->write(instanceUuids);

      HighFive::DataSpace boxesDataSpace =
          HighFive::DataSpace::From(boundingBoxesWithRotation);
      auto datasetBoxes = getHdf5DataSet<double>(
          dataGroupId + "/" +
              seerep_hdf5_core::Hdf5CoreGeneral::LABELBBBOXESWITHROTATION +
...
Hdf5PyGeneral::readBoundingBoxLabeled(const std::string& dataGroupId)
{
  ...
  {
    if (datasetName.rfind(seerep_hdf5_core::Hdf5CoreGeneral::LABELBBINSTANCES,
                          0) == 0)
    {
      std::string category = datasetName.substr(
          seerep_hdf5_core::Hdf5CoreGeneral::LABELBBINSTANCES.length() + 1);
      ...
      auto datasetLabelConfidences =
          getHdf5DataSet(dataGroupId + "/" +
                         seerep_hdf5_core::Hdf5CoreGeneral::LABELBBCONFIDENCES +
                         "_" + category);
      auto datasetInstances = getHdf5DataSet(
          dataGroupId + "/" +
          seerep_hdf5_core::Hdf5CoreGeneral::LABELBBINSTANCES + "_" + category);
      auto datasetBoxes = getHdf5DataSet(
          dataGroupId + "/" +
          seerep_hdf5_core::Hdf5CoreGeneral::LABELBBBOXESWITHROTATION + "_" +
          category);

      if (datasetLabels == nullptr || datasetLabelConfidences == nullptr ||
          datasetInstances == nullptr || datasetBoxes == nullptr)
      {
        throw std::invalid_argument(
            "cannot find all bounding box label datasets for category " +
            category);
      }
      ...
      {
        throw std::invalid_argument(
            "amounts for labels, instance uuids and bounding boxes do not "
            "match for category " +
            category);
      }
      ...
      for (std::size_t i = 0; i < labels.size(); i++)
      {
        ...
        if (boundingBoxesWithRotation[i].size() !=
            2 * NumDimensions + bbRotation.size())
        {
          throw std::invalid_argument(
              std::to_string(NumDimensions) +
              "-dimensional bounding box expected for category " + category +
              " but got " +
              std::to_string(boundingBoxesWithRotation[i].size() / 2 -
                             bbRotation.size()) +
              "-dimensional one");
        }
        ...
        BoundingBoxLabel<NumDimensions> bbLabel(label, bbCenter,
                                                bbSpatialExtent, bbRotation);
