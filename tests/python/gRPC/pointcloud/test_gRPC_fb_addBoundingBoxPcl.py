...
def test_addLabelToPCs(grpc_channel, project_setup):
    ...
    for label_pc_uuid, label_pc in sent_label:
        pc_label = [pc for pc in queried_pcs if pc["header"]["uuid_msgs"] == label_pc_uuid][0]

        # iterate through all categories of the image
        filtered_label = [label for label in pc_label["labels"] if label["category"] == "laterAddedLabel"]

        sent_labels = fb_flatc_dict(label_pc, SchemaFileNames.DATASET_UUID_LABEL)["labels"]
