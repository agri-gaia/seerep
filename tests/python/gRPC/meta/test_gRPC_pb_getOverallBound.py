...
def test_gRPC_pb_getOverallBound(grpc_channel, project_setup) -> None:
    ...
    # get information such as timestamp, boundingbox, category, all labels of
    # images in a specified project
    ...
    for inner_lst in sent_grid:
        for img_lst in inner_lst:
            for _, img in img_lst:
                timestamps.add(
                    (img.header.stamp.seconds, img.header.stamp.nanos)
                )
    ...
    for img in imgs_sorted:
        ...
        while True:
            img_time = (
                img.header.stamp.seconds + img.header.stamp.nanos * NANOS_FACTOR
            )
            if img_time == curr_ts:
                coordinate_trans = (
                    sorted_tfs[curr_idx].Transform().Translation()
                )
            ...
            # else search next tf stamp via binary search
                ...
                curr_ts = (
                    ...
                    + sorted_tfs[curr_idx].Header().Stamp().Nanos()
                    * NANOS_FACTOR
                )
                if curr_idx - 1 >= 0:
                    prev_ts = (
    ...
    # cause then the floating point numbers of resp_overall_bb are not exactly
    # zero
    epsilon = 1e-34
