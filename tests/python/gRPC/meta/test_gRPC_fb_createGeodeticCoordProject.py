...
def test_gRPC_fb_createGeoProjectAndGetProjects(grpc_channel):
    ...
    current_projects = remap(
        fb_flatc_dict(
            projects_query.get_projects_raw(grpc_channel),
            SchemaFileNames.PROJECT_INFOS,
        )["projects"],
    ...
    created_project = remap(
        fb_flatc_dict(
            project_creation.create_geo_proj_raw(grpc_channel),
            SchemaFileNames.PROJECT_INFO,
        ),
        visit=common.remap_to_snake_case,
    )
    ...
    # projects, returned by the gRPC_pb_getProjects example
    ...
    fb_helper.deleteProject(
        grpc_channel, builder, created_project["name"], created_project["uuid"]
    )
