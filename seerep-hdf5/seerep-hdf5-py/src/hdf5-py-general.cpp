...
void Hdf5FileWrapper::setProjectGeolocation(const std::string& coordinateSystem, const std::string& ellipsoid,
                                            double latitude, double longitude, double altitude)
{
  Hdf5PyGeneral io(*this);

  io.setProjectGeolocation(coordinateSystem, ellipsoid, latitude, longitude, altitude);
}

Hdf5PyGeneral::Hdf5PyGeneral(Hdf5FileWrapper& hdf5File) : Hdf5CoreGeneral(hdf5File.getFile(), hdf5File.getMutex())
{
}

void Hdf5PyGeneral::setProjectGeolocation(const std::string& coordinateSystem, const std::string& ellipsoid,
                                          double latitude, double longitude, double altitude)
{
...
void Hdf5PyGeneral::writeLabelsGeneral(const std::string& dataGroupId,
  ...
  for (const auto& categoryLabels : generalLabels)
  {
    if (!categoryLabels.labels_.empty())
    {
      ...
      auto datasetInstances = getHdf5DataSet<std::string>(
          dataGroupId + "/" + seerep_hdf5_core::Hdf5CoreGeneral::LABELGENERALINSTANCES + "_" + categoryLabels.category_,
          instancesDataSpace);
...
std::vector<GeneralLabel> Hdf5PyGeneral::readLabelsGeneral(const std::string& dataGroupId)
{
  ...
  {
    if (datasetName.rfind(seerep_hdf5_core::Hdf5CoreGeneral::LABELGENERALINSTANCES, 0) == 0)
    {
      std::string category = datasetName.substr(seerep_hdf5_core::Hdf5CoreGeneral::LABELGENERALINSTANCES.length() + 1);
