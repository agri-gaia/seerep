...
{
TfIOCore::TfIOCore(std::shared_ptr<HighFive::File>& file, std::shared_ptr<std::mutex>& write_mtx)
  : GeneralIOCore(file, write_mtx)
{
}

std::optional<std::vector<geometry_msgs::TransformStamped>> TfIOCore::readTransformStamped(const std::string& id)
{
...
std::optional<std::vector<std::string>> TfIOCore::readTransformStampedFrames(const std::string& id)
{
  std::string hdf5GroupPath = HDF5_GROUP_TF + "/" + id;
