...
void CoreProject::createHdf5Io(boost::uuids::uuid& uuid, std::string path)
{
  ...
  m_ioGeneral = std::make_shared<seerep_io_core::IoCoreGeneral>(m_hdf5_file, m_write_mtx);
  m_ioTf = std::make_shared<seerep_io_core::IoCoreTf>(m_hdf5_file, m_write_mtx);
  // m_ioPointCloud = std::make_shared<seerep_io_core::PointCloudIOCore>(hdf5_file, m_write_mtx);
  m_ioImage = std::make_shared<seerep_io_core::IoCoreImage>(m_hdf5_file, m_write_mtx);
}
