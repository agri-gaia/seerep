...
Image::Image(std::shared_ptr<seerep_hdf5::ImageIO> hdf5_io, const seerep::Image& image, const uint64_t& id,
  ...
  : m_hdf5_io(hdf5_io), m_id(id), m_uuid(uuid)
{
  ...
  // space
  m_aabb = calcAABB();
  m_hdf5_io->writeAABB(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid), m_aabb);
...
Image::Image(std::shared_ptr<seerep_hdf5::ImageIO> hdf5_io, const uint64_t& id, const boost::uuids::uuid& uuid)
  : m_hdf5_io(hdf5_io), m_id(id), m_uuid(uuid)
{
  auto frameId =
      m_hdf5_io->readFrameId(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid));
...
{
  if (m_hdf5_io->hasAABB(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid)))
  {
    m_hdf5_io->readAABB(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid), m_aabb);
  }
  else
  {
    m_aabb = calcAABB();
...
{
  if (m_hdf5_io->hasTime(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid)))
  {
    m_hdf5_io->readTime(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid), m_timeSecs,
                        m_timeNanos);
  }
  else
  {
    if (m_hdf5_io->hasTimeRaw(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid)))
    {
      m_hdf5_io->readTimeFromRaw(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid),
                                 m_timeSecs, m_timeNanos);
      m_hdf5_io->writeTime(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid), m_timeSecs,
                           m_timeNanos);
    }
  }
}
...
void Image::recreateLabel()
{
  std::optional<google::protobuf::RepeatedPtrField<std::string>> labelGeneral =
      m_hdf5_io->readLabelsGeneral(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE, boost::lexical_cast<std::string>(m_uuid));
  if (labelGeneral)
  {
    storeLabelGeneral(labelGeneral.value());
  }
  std::optional<google::protobuf::RepeatedPtrField<seerep::BoundingBox2DLabeled>> labelsBB =
      m_hdf5_io->readBoundingBox2DLabeled(seerep_hdf5::ImageIO::HDF5_GROUP_IMAGE,
