...
TransferImagesWithDetection::TransferImagesWithDetection(
    std::shared_ptr<grpc::Channel> channel_ptr, std::string categoryLabels)
...
TransferImagesWithDetection::~TransferImagesWithDetection()
{
  ...
  ROS_INFO_STREAM(
      "tf transfer server response: " << tfResponse_.GetRoot()->message());
  ...
  ROS_INFO_STREAM("image transfer server response: "
                  << imageResponse_.GetRoot()->message());
}

void seerep_grpc_ros::TransferImagesWithDetection::send(
    const sensor_msgs::Image::ConstPtr& msg)
{
  ...
  if (!writerImage_->Write(
          seerep_ros_conversions_fb::toFlat(*msg, projectuuid_, uuidstring)))
  {
    ...
    const vision_msgs::Detection2DArray::ConstPtr& msg)
{
  ...
  // }
  ROS_INFO_STREAM("transfering detections to server: "
                  << uuidstring << " time: " << msg->header.stamp.sec << " / "
                  << msg->header.stamp.nsec);
    ...
    const sensor_msgs::NavSatFix::ConstPtr& msg)
{
  ...
  if (!localCartesian_)
  {
    localCartesian_ = std::move(std::make_unique<GeographicLib::LocalCartesian>(
        msg->latitude, msg->longitude, msg->altitude));
  }
  double x, y, z;
  localCartesian_->Forward(msg->latitude, msg->longitude, msg->altitude, x, y,
                           z);
  ...
  if (!writerTf_->Write(
          seerep_ros_conversions_fb::toFlat(transform, projectuuid_, false)))
  {
...
void TransferImagesWithDetection::createSubscriber()
{
  ...
  subscribers_.emplace("/camera/color/Detection2DArray",
                       nh_.subscribe<vision_msgs::Detection2DArray>(
                           "/camera/color/Detection2DArray", 10,
                           &TransferImagesWithDetection::send, this));
  subscribers_.emplace("/fix", nh_.subscribe<sensor_msgs::NavSatFix>(
                                   "/fix", 10,
                                   &TransferImagesWithDetection::send, this));
}
...
void TransferImagesWithDetection::createProject()
{
  ...
  auto status =
      stubMeta_->CreateProject(&contextMeta, request_msg, &response_msg);

  // store answer
  projectuuid_ = response_msg.GetRoot()->uuid()->str();
  ROS_INFO_STREAM("Project created: " << response_msg.GetRoot()->name()->str()
                                      << " " << projectuuid_);
}
}  // namespace seerep_grpc_ros
...
{
  ros::init(argc, argv,
            "seerep_ros_communication_client_transferImagesWithDetection");
