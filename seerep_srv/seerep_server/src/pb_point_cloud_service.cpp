...
{
PbPointCloudService::PbPointCloudService(
    std::shared_ptr<seerep_core::Core> seerepCore)
  : pointCloudPb(std::make_shared<seerep_core_pb::CorePbPointCloud>(seerepCore))
{
}

grpc::Status PbPointCloudService::GetPointCloud2(
    grpc::ServerContext* context, const seerep::pb::Query* request,
    grpc::ServerWriter<seerep::pb::PointCloud2>* writer)
{
  ...
  try
  {
    ...
    {
      BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::debug)
          << "Found " << pointClouds.size()
          << " pointclouds that match the query";
    ...
    {
      BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::debug)
          << "Found NOTHING that matches the query";
    }
  }
...
grpc::Status PbPointCloudService::TransferPointCloud2(
    grpc::ServerContext* context, const seerep::pb::PointCloud2* pointCloud2,
    seerep::pb::ServerResponse* response)
{
  (void)context;  // ignore that variable without causing warnings
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::debug)
      << "received point clouds... ";
  ...
  if (!pointCloud2->header().uuid_project().empty())
  {
    try
    {
      ...
      seerep_server_util::createResponsePb(
          boost::lexical_cast<std::string>(pointCloudUuid),
