...
{
FbCameraIntrinsicsService::FbCameraIntrinsicsService(
    std::shared_ptr<seerep_core::Core> seerepCore)
  : ciFbCore(
        std::make_shared<seerep_core_fb::CoreFbCameraIntrinsics>(seerepCore))
{
}
    ...
    flatbuffers::grpc::Message<seerep::fb::CameraIntrinsics>* response)
{
  ...
  if (requestRoot->uuid_camera_intrinsics() != NULL)
  {
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::trace)
        << "fetching camera intrinsics against camera intrinsics uuid "
        << requestRoot->uuid_camera_intrinsics()->str();
  }
  if (requestRoot->uuid_project() != NULL)
  {
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::trace)
        << "fetching camera intrinsics against project uuid "
        << requestRoot->uuid_project()->str();
  }
    ...
    flatbuffers::grpc::Message<seerep::fb::ServerResponse>* response)
{
  ...
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
      << "received camera instrinsics for storage ";
  ...
  bool distortion_model_err =
      ...
      std::find(std::begin(seerep_server_constants::kCameraDistortionModels),
                ...
                requestRoot->distortion_model()->c_str()) ==
          std::end(seerep_server_constants::kCameraDistortionModels);

  bool intrinsics_mat_err = !requestRoot->intrinsic_matrix() ||
                            requestRoot->intrinsic_matrix()->size() != 9 ||
                            (*requestRoot->intrinsic_matrix())[0] == 0 ||
                            (*requestRoot->intrinsic_matrix())[4] == 0;

  bool rectification_mat_err =
      requestRoot->rectification_matrix() &&
      (requestRoot->rectification_matrix()->size() > 0 &&
       requestRoot->rectification_matrix()->size() != 9);

  bool projection_mat_err = requestRoot->projection_matrix() &&
                            (requestRoot->projection_matrix()->size() > 0 &&
      ...
      projection_mat_err)
  {
    ...
    {
      response_message += "Instrinsics matrix is not set, dimensions are "
                          "incorrect or the focal length values are "
                          "invalid. ";
    }
    ...
    seerep_server_util::createResponseFb(
        response_message, seerep::fb::TRANSMISSION_STATE_FAILURE, response);
  ...
  {
    std::string msg =
        "Max Viewing Distance is not set. Using default value of 0.";
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::error) << msg;

    response_message = "Max Viewing Distance is the distance to the farthest "
  ...
  seerep_server_util::createResponseFb(
      response_message, seerep::fb::TRANSMISSION_STATE_SUCCESS, response);
