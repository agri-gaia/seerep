...
void server::signalHandler(int signum)
{
  std::cout << "Interrupt signal (" << signum
            << ") received. Flushing log file." << std::endl;
...
void server::parseProgramOptions(int argc, char** argv)
{
  try
  {
    ...
    generic.add_options()("version,v", "Get a version string")(
        "help", "Help message")("config,c",
    ...
    config.add_options()(
        ...
        "port,p",
        boost::program_options::value<std::string>()->default_value("9090"),
        "gRPC port to use");
    ...
    store(boost::program_options::parse_environment(
              config, environmentVariabeNameMapper),
          m_programOptionsMap);
    ...
    if (m_programOptionsMap.count("config"))
    {
      ...
      if (!ifs)
      {
        std::cout << "can not open config file: "
                  << m_programOptionsMap["config"].as<std::string>()
                  << std::endl;
        exit(EXIT_FAILURE);
      }
      else
...
void server::initLogging()
{
  ...
  {
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
        << "The used logging folder is: " << logPath;
  }
  catch (std::exception& e)
  {
...
void server::logTimeZone()
{
  ...
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
      << "Current timezone: " << buf;
}

void server::logServerVersion()
{
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
      << "SEEREP version: " << GIT_TAG;
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::debug)
      << "SEEREP git commit hash: " << GIT_REV;
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::debug)
      << "SEEREP git branch: " << GIT_BRANCH;
}
...
std::string server::initFileLogging()
{
  ...
  if (!logPath.empty())
  {
    boost::log::add_file_log(
        boost::log::keywords::file_name = logPath + "seerep_%N.log",
...
void server::createGrpcServer()
{
  ...
  std::string serverAddress =
      "[::]:" + m_programOptionsMap.at("port").as<std::string>();
  grpc::ServerBuilder serverBuilder;
  serverBuilder.AddListeningPort(serverAddress,
...
std::string server::getDataFolder()
{
  try
  {
    std::string dataFolder =
        m_programOptionsMap.at("data-folder").as<std::string>();
    ...
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
        << "The used data folder is: " << dataFolder;
  ...
  {
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::fatal)
        << e.what();
    exit(EXIT_FAILURE);
  }
}
...
void server::addServicesPb(grpc::ServerBuilder& server_builder)
{
  ...
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
      << "Addded Protocol Buffers gRPC services";
}
...
void server::addServicesFb(grpc::ServerBuilder& server_builder)
{
  ...
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
      << "Added Flatbuffers gRPC services";
}

void server::createServicesPb()
{
  m_metaOperationsPb =
      std::make_shared<seerep_server::PbMetaOperations>(m_seerepCore);
  m_tfServicePb = std::make_shared<seerep_server::PbTfService>(m_seerepCore);
  m_imageServicePb =
      std::make_shared<seerep_server::PbImageService>(m_seerepCore);
  m_pointCloudServicePb =
      std::make_shared<seerep_server::PbPointCloudService>(m_seerepCore);
  m_cameraIntrinsicsServicePb =
      std::make_shared<seerep_server::PbCameraIntrinsicsService>(m_seerepCore);
}

void server::createServicesFb()
{
  m_metaOperationsFb =
      std::make_shared<seerep_server::FbMetaOperations>(m_seerepCore);
  m_tfServiceFb = std::make_shared<seerep_server::FbTfService>(m_seerepCore);
  m_instanceServiceFb =
      std::make_shared<seerep_server::FbInstanceService>(m_seerepCore);
  m_imageServiceFb =
      std::make_shared<seerep_server::FbImageService>(m_seerepCore);
  m_pointServiceFb =
      std::make_shared<seerep_server::FbPointService>(m_seerepCore);
  m_pointCloudServiceFb =
      std::make_shared<seerep_server::FbPointCloudService>(m_seerepCore);
  m_cameraIntrinsicsServiceFb =
      std::make_shared<seerep_server::FbCameraIntrinsicsService>(m_seerepCore);
}
