...
{
CoreFbPointCloud::CoreFbPointCloud(std::shared_ptr<seerep_core::Core> seerepCore)
  : m_seerepCore(seerepCore)
{
  CoreFbGeneral::getAllFileAccessorFromCore(m_seerepCore, m_hdf5IoMap);
}

void CoreFbPointCloud::getData(
    const seerep::fb::Query* query,
    grpc::ServerWriter<flatbuffers::grpc::Message<seerep::fb::PointCloud2>>* const
        writer)
{
  seerep_core_msgs::Query queryCore = seerep_core_fb::CoreFbConversion::fromFb(
      query, seerep_core_msgs::Datatype::PointCloud);
  ...
  for (auto project : resultCore.queryResultProjects)
  {
    BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::debug)
        << "sending flatbuffer point cloud from project: "
    ...
    for (auto uuidPc : project.dataOrInstanceUuids)
    {
      ...
      {
        BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::debug)
            << "sending flatbuffers point cloud with UUID: "
            << boost::lexical_cast<std::string>(uuidPc);
...
boost::uuids::uuid
CoreFbPointCloud::addDataToHdf5(const seerep::fb::PointCloud2& pcl)
{
  seerep_core_msgs::DatasetIndexable dataForIndices =
      CoreFbConversion::fromFb(pcl);
  ...
  hdf5io->writePointCloud2(
      boost::lexical_cast<std::string>(dataForIndices.header.uuidData), pcl);
  hdf5io->writeBoundingBox(
      boost::lexical_cast<std::string>(dataForIndices.header.uuidData),
      min_corner, max_corner);

  return dataForIndices.header.uuidData;
}

void CoreFbPointCloud::buildIndices(
    std::vector<std::pair<std::string, boost::uuids::uuid>>& projectPclUuids)
{
  ...
  for (auto [projectUuid, pclUuid] : projectPclUuids)
  {
    ...
    hdf5io->readAABB(seerep_hdf5_core::Hdf5CorePointCloud::HDF5_GROUP_POINTCLOUD,
                     uuidStr, bb);
...
void CoreFbPointCloud::addLabel(
    const seerep::fb::DatasetUuidLabel& datasetUuidLabel)
{
  ...
  m_seerepCore->addLabels(seerep_core_msgs::Datatype::PointCloud,
                          labelPerCategory, uuidMsg, uuidProject);
}
