...
{
CorePbImage::CorePbImage(std::shared_ptr<seerep_core::Core> seerepCore)
...
void CorePbImage::getData(const seerep::pb::Query& query,
                          grpc::ServerWriter<seerep::pb::Image>* const writer)
{
  BOOST_LOG_SEV(m_logger, boost::log::trivial::severity_level::info)
      << "loading image from images/" << std::endl;
  seerep_core_msgs::Query queryCore =
      CorePbConversion::fromPb(query, seerep_core_msgs::Datatype::Image);
  ...
  for (auto project : resultCore.queryResultProjects)
  {
    ...
    for (auto uuidImg : project.dataOrInstanceUuids)
    {
      ...
      std::optional<seerep::pb::Image> image =
...
boost::uuids::uuid CorePbImage::addData(const seerep::pb::Image& img)
{
  ...
  if (m_seerepCore->cameraIntrinsicExists(camintrinsics_query))
  {
    ...
    hdf5io->computeFrustumBB(img.uuid_camera_intrinsics(),
...
CorePbImage::getHdf5(boost::uuids::uuid project)
{
