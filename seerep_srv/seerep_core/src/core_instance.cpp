  ...
  : m_hdf5_io(hdf5_io), m_uuid(uuidInstance)
{
  ...
  m_typeUUIDMap.emplace(seerep_core_msgs::Datatype::Image,
                        std::vector<boost::uuids::uuid>());
  m_typeUUIDMap.emplace(seerep_core_msgs::Datatype::PointCloud,
                        std::vector<boost::uuids::uuid>());
  m_typeUUIDMap.emplace(seerep_core_msgs::Datatype::Point,
                        std::vector<boost::uuids::uuid>());
}
CoreInstance::~CoreInstance()
{
...
CoreInstance::getAttribute(const std::string& key) const
{
...
void CoreInstance::writeAttribute(const std::string& key,
...
std::vector<boost::uuids::uuid>
CoreInstance::getDatasets(const seerep_core_msgs::Datatype& datatype) const
{
  return m_typeUUIDMap.at(datatype);
}

void CoreInstance::addDataset(const boost::uuids::uuid& uuidDataset,
